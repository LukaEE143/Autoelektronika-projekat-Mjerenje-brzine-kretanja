...
static uint32_t prvProcessRXCInterrupt(void) //koristi se za detekciju dolaska novih podataka na serjiskim kanalima 0 i 1 i signalizaciju taska koji ceka na te podatke preko semafora  
{
	BaseType_t xHigherPTW = pdFALSE; //Kreira se promenljiva xHigherPTW koja se koristi za praćenje da li je bilo višeg prioriteta prekida tokom izvršenja ove funkcija
	...
	portYIELD_FROM_ISR((uint32_t)xHigherPTW); //ovom funkcijom se trazi od raspređivačanda se izvrši promjena konteksta, ali samo ako je kao argument prosleđen pdTRUE
}
...
static void SerialReceive_Task0(void* pvParameters)
{
	...
	while (1)
	{
		...
		else if (cc == (uint8_t)'.')
		{
			...
			xQueueSend(inkrementi_q, &inkrementi, 0); //0 predstavlja max vrijeme cekanja 
...
static void SerialSend_Task1(void* pvParameters)
{
	...
	while (1)
	{
		...
		//razlaganje ukupnog puta na pojedinacne cifre, odnosno dekompozicija
		...
		/*ispisivanje puta u prozoru kanala 1*/
		...
		else
		{
			...
			if (ukupni_put != (uint8_t)0)
			{
				...
				if (counter >= (uint8_t)1 && counter <= (uint8_t)8)
				{

					send_serial_character(COM_CH_1, ispis_puta[counter - 1]);

					if (counter == (uint8_t)8)
					{
						counter = (uint8_t)0;
					}
				}
...
static void led_bar_tsk(void* pvParameters)
{
	...
	while (1)
	{
		...
		{//postavljanje svih vrijednosti 7seg displeja na 0
			for (i = 0; i < 9; i++)
			{
...
static void mjerenje_brzine(void* pvParameters)
{
	...
	while (1)
	{
		...
		ispis_brzine[0] = (uint16_t)hiljada + (uint16_t)'0'; /*dodavanjem '0' se vrsi konverzija u ASCII karakter */
...
static void ukupni_predjeni_put(void* pvParameters)
{
	...
	while (1)
	{
		...
		if (d == (uint8_t)1) /*sluzi da se resetuje put, kad se dioda ukljuci pa zatim iskljuci */
		{
			obrtaji = (uint16_t)0;
